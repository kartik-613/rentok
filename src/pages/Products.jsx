...
const Products = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <h2 className="text-2xl sm:text-3xl font-bold capitalize mb-8 text-gray-800 text-center">
        {category.replace("-", " ")} for Rent
      </h2>

      <div className="flex flex-col lg:flex-row gap-8">
        {/* Filters Sidebar */}
        <aside className="lg:w-1/4 w-full bg-white rounded-xl shadow-md p-5 h-fit sticky top-5">
            ...
            {['Maharashtra', 'Tamil Nadu', 'Kerala', 'Delhi'].map((loc) => (
              <label className="block text-sm text-gray-700" key={loc}>
                <input
                  type="checkbox"
                  value={loc}
                  onChange={(e) => handleCheckboxChange(e, locationFilter, setLocationFilter)}
                /> {loc}
              </label>
            ))}
          </div>
          ...
          {/* Fuel Type */}
          <div className="mb-6">
            <h4 className="font-medium text-sm text-gray-600 mb-2">FUEL TYPE</h4>
            {['Petrol', 'Diesel', 'CNG', 'Electric'].map((fuel) => (
              <label className="block text-sm text-gray-700" key={fuel}>
                <input
                  type="checkbox"
                  value={fuel}
                  onChange={(e) => handleCheckboxChange(e, fuelFilter, setFuelFilter)}
                /> {fuel}
              </label>
            ))}
          </div>
          ...
          {/* Transmission */}
          <div className="mb-6">
            <h4 className="font-medium text-sm text-gray-600 mb-2">TRANSMISSION</h4>
            {['Automatic', 'Manual'].map((t) => (
              <label className="block text-sm text-gray-700" key={t}>
                <input
                  type="checkbox"
                  value={t}
                  onChange={(e) => handleCheckboxChange(e, transmissionFilter, setTransmissionFilter)}
                /> {t}
              </label>
            ))}
          </div>
          ...
          {/* Sort By */}
          <div className="mb-4">
            <h4 className="font-medium text-sm text-gray-600 mb-2">SORT BY</h4>
            <select
